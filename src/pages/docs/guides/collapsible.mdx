...
<Callout title="Extension guide">
  This guide is a reference for multi-node block features that own both document
...
The root of the collapsible structure. Stores a single boolean `__open` and exposes `toggleOpen()`. Acts as a shadow root so inner selections stay contained.
...
Renders as a `<summary>` element. On Chrome, a click listener calls `container.toggleOpen()` via `editor.update()`. Pressing Enter in the title jumps into the content body.
...
The body area. On Chrome uses `setDomHiddenUntilFound()` so the browser's Ctrl+F can still find text inside collapsed sections. Acts as a shadow root.
...
Registers node transforms to enforce the `container → [title, content]` invariant, four arrow-key handlers for cursor escape, an `INSERT_PARAGRAPH_COMMAND` handler to open the body on Enter in the title, and the `INSERT_COLLAPSIBLE_COMMAND` handler.
...
The collapsible capability ships behind the `collapsible` feature flag. To add it to a custom composition, contribute a descriptor via `extraFeatures`:
