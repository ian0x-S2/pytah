...
<Callout title="Extension guide">
  ...
  feature plugin. It is a good reference when adding new structural block types
  to the editor.
</Callout>
...
Renders as a `<div>` with `data-lexical-layout-container` and `style.gridTemplateColumns` set to the stored template string (e.g. `"1fr 1fr"`). Acts as a shadow root.
...
Each column is a `LayoutItemNode`. When the user backspaces at the start of the first column and all columns are empty, the entire layout container is removed via `collapseAtStart()`.
...
Predefined grid templates. `DEFAULT_LAYOUT_TEMPLATE` is used by the slash-command executor.
...
Layouts ship behind the `layouts` feature flag. To add the capability to a custom composition, contribute a descriptor via `extraFeatures`:
