...
<Callout title="Feature guide">
  This page describes a built-in product capability. Use the `features` prop on
  `Editor` if you only need to enable or disable it from a consumer integration.
</Callout>
...
A dedicated Lexical command that tells `FloatingLinkEditorPlugin` to open in edit mode. Kept in a separate file so the floating toolbar and link editor can import it without a circular dependency.
...
Pure read-only helpers that compute the toolbar's state from the current Lexical selection. Called inside `editor.getEditorState().read()`.
...
Imperative helpers that dispatch Lexical commands or call `editor.update()`. Imported by the plugin and by the floating link editor.
...
The plugin subscribes to `SELECTION_CHANGE_COMMAND` and the update listener, then renders the toolbar as a fixed portal over the selection. A `isColorPickerOpenRef` prevents the toolbar from disappearing while a color popover is open.
...
The floating toolbar ships behind the `floatingToolbar` feature flag. To add it to a custom composition, contribute a descriptor via `extraFeatures`:
